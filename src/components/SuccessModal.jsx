...
const EASE = [0.22, 1, 0.36, 1]; // same curve used across the site's loader/founding-batch — one consistent motion system
...
export default function SuccessModal({
  ...
  const courseTitle =
    COURSE_TITLES[primaryCourseName] || primaryCourseName || "Tech Program";
  ...
  useEffect(() => {
    ...
    const recalcScale = () => {
      ...
      const scale =
        naturalWidth > 0 ? Math.min(1, availableWidth / naturalWidth) : 1;
      setCardScale(scale);
    ...
    const fetchQr = async () => {
      try {
        const res = await fetch(
          `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${appId}`,
        );
  ...
  const buildAdmitCardPDF = () => {
    ...
    pdf.text("Official Digital Student Admit Card & ID", 105, 23, {
      align: "center",
    });
    ...
    const detailRows = [
      ...
      { label: "DEPARTMENT", value: "Technology" },
      {
        label: "EMAIL",
        value:
          form.email.length > 25 ? form.email.slice(0, 23) + ".." : form.email,
      },
    ...
    pdf.text("LEARN • BUILD • SUCCEED", cx, fc.y + fc.h - 7, {
      align: "center",
    });
    ...
    pdf.setFontSize(5.5);
    pdf.setTextColor(37, 99, 235);
    pdf.text("LEARN • BUILD • SUCCEED", bcx, bc.y + 21.5, {
      align: "center",
    });
    ...
    const textLineH = 3;
    const terms = [
      "\u2022  Report loss or damage to management.",
      "\u2022  This ID card is strictly non-transferable.",
      "\u2022  This ID card is the property of Shine Tech Academy.",
      "\u2022  Return card when no longer associated with the academy.",
    ];
    ...
    pdf.setFontSize(6);
    pdf.setTextColor(255, 255, 255);
    pdf.text("TERMS & CONDITIONS", bcx, badgeY + badgeH / 2 + 0.75, {
      align: "center",
    });
    ...
    pdf.setFontSize(12);
    pdf.setTextColor(30, 41, 59);
    pdf.text("", bc.x + bc.w - 6, bc.y + 98, { align: "right" });
    pdf.setDrawColor(203, 213, 225);
    pdf.setLineWidth(0.3);
    ...
    pdf.setFontSize(5.5);
    pdf.setTextColor(148, 163, 184);
    pdf.text("AUTHORIZED SIGNATURE", bc.x + bc.w - 6, bc.y + 105, {
      align: "right",
    });
    ...
    pdf.text(
      "+923351866930  \u2022  info@shinetechacademy.com",
      bcx,
      bc.y + bc.h - 10,
      { align: "center" },
    );
    pdf.setTextColor(148, 163, 184);
    pdf.text(
      "Bungalow No. 306, Unit No. 9, Opposite NADRA Latifabad, Hyd",
      bcx,
      bc.y + bc.h - 5,
      {
        align: "center",
    ...
    const instrLines = [
      ...
      "2.  Please print this Admit Card on a high-quality standard A4 page or card stock.",
      "3.  Carry this Admit Card with you to all classes, examinations, and official academy activities.",
      "4.  This Admit Card is issued to the registered student and is strictly non-transferable.",
      "5.  The QR Code on the back may be scanned by Shine Tech Academy management to verify your student status.",
      "6.  In case of loss, damage, or any admission-related issue, please contact Shine Tech Academy administration.",
    ];
    let iy = 189;
    ...
    pdf.setFontSize(7);
    pdf.setTextColor(148, 163, 184);
    pdf.text("SHINE TECH ACADEMY | www.shinetechacademy.com", 105, 282, {
      align: "center",
    });
  ...
  useEffect(() => {
    const uploadAdmitCard = async () => {
      ...
      try {
        ...
        await supabase
          .from("admissions")
          .update({ admit_card_url: publicUrl })
          .eq("app_id", appId);
      } catch (err) {
      ...
      >
        <div className="p-4 sm:p-6 lg:p-8 overflow-y-auto flex-1">
          ...
          <div className="text-center mb-6">
            <motion.div
              ...
              className="w-14 h-14 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4 text-green-600"
            >
            ...
            <motion.h3
              ...
              className="font-space font-extrabold text-slate-900 text-2xl mb-1"
            >
              Admission Submitted!
            </motion.h3>
            <motion.p
            ...
            >
              Your admission request has been sent successfully. Your digital
              student admit card is ready.
            </motion.p>

            {/* Sync status — makes the background upload visible instead of silent */}
              ...
              <motion.div
                ...
                className={`inline-flex items-center gap-1.5 mt-3 text-xs font-semibold px-3 py-1.5 rounded-full ${
                  uploadError
                    ...
                    : "bg-blue-50 text-[#0956fc] border border-blue-200/80"
                }`}
              >
                {uploadError ? (
                  <>
                    <AlertTriangle className="w-3.5 h-3.5" />
                    Couldn't sync to your profile — you can still download below
                  </>
                ) : (
                  <>
                    <CloudUpload className="w-3.5 h-3.5 animate-pulse" />
                    Saving your admit card...
                  </>
                )}
              </motion.div>
            )}
          ...
          >
            <motion.div
              ...
              style={{
                ...
                width: "max-content",
                marginBottom: cardsMarginBottom,
                ...
                >
                  <div
                    style={{
                      ...
                      pointerEvents: "none",
                      backgroundImage:
                        "radial-gradient(#0956fc 1px, transparent 1px)",
        ...
        <div className="flex flex-col sm:flex-row gap-3 pt-6 border-t border-slate-100 p-8 bg-slate-50">
          <button
            ...
            className="group relative flex-1 flex items-center justify-center gap-2 bg-[#0956fc] hover:bg-blue-700 text-white font-bold text-sm py-3.5 rounded-xl transition-all shadow-lg shadow-blue-500/20 disabled:opacity-60 disabled:cursor-not-allowed overflow-hidden"
          >
            {!downloading && (
          ...
          <button
            onClick={onClose}
            className="sm:w-44 bg-white border border-slate-200 text-slate-700 font-bold text-sm py-3.5 rounded-xl hover:bg-slate-100 hover:border-slate-300 transition-colors"
          >
            Return to Home
          </button>
        </div>
